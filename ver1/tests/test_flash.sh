#!/usr/bin/env bash
# test_flash.sh - Flash module tests
set -euo pipefail
...
assert_eq "probe tool" "fio" "$(kv_read "$TMPDIR/probe1/probe.kv" "tool")"

# Test 2: Run
mkdir -p "$TMPDIR/run1"
bash "$MODULE" run --out "$TMPDIR/run1" --duration 1 --cfg "$TMPDIR/config.kv" > "$TMPDIR/run1/stdout.log" 2>&1
grep -q "fio\|IOPS\|syshammer" "$TMPDIR/run1/stdout.log"
assert_eq "run produces fio output" "0" "$?"

# Test 3: Evaluate clean
mkdir -p "$TMPDIR/eval1"
echo "fio: IOPS=1234" > "$TMPDIR/eval1/stdout.log"
...
# Test 4: Evaluate with IO error in dmesg
mkdir -p "$TMPDIR/eval2"
echo "done" > "$TMPDIR/eval2/stdout.log"
export FAKE_DMESG_OUTPUT="[  10.000] blk_update_request: I/O error, dev sda, sector 12345"
...
assert_eq "IO error detected" "0" "$?"

# Test 5: Evaluate with verify failure
mkdir -p "$TMPDIR/eval3"
echo "VERIFY_FAIL: bad checksums detected" > "$TMPDIR/eval3/stdout.log"
DURATION=10 bash "$MODULE" evaluate --out "$TMPDIR/eval3" --cfg "$TMPDIR/config.kv"
grep -q "IO_ERROR" "$TMPDIR/eval3/fails.kv"
assert_eq "verify fail detected" "0" "$?"

echo ""
echo "test_flash: $PASS/$TOTAL passed, $FAIL failed"
[[ $FAIL -eq 0 ]]
