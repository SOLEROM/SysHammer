#!/usr/bin/env bash
# bus_i2c/bus_i2c.sh - i2cget/i2cset transactions or skip
set -euo pipefail
...
cmd_help() {
    cat <<'EOF'
bus_i2c/bus_i2c.sh - I2C read transaction testing via i2cget

Usage: bus_i2c.sh <command> [options]
...
Examples:
  ./bus_i2c.sh probe --cfg my_config.kv
  ./bus_i2c.sh run --duration 10 --cfg my_config.kv
EOF
}
